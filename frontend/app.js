...
// ── Calculator-only init (skipped on non-calculator pages) ──────────────────
if ($('computeBtn')) {
  // ── Drop zone wiring ──────────────────────────────────────────────────────────
...
// ── State ─────────────────────────────────────────────────────
let DB_DATA_3D  = [];   // 3D bulk materials (db.json)
let DB_DATA_2D  = [];   // 2D monolayer materials (db2d.json)
let DB_DATA     = [];   // active dataset (points to one of the above)
let db_dim      = '3D'; // '3D' | '2D'
let db_selected = new Set();
let db_filtered = [];
let db_page     = 1;
const DB_PER_PAGE = 50;
let db_sort       = 'desc';
let db_search     = '';
let db_filter_mode = 'at-least';

// ── Source → URL builder ──────────────────────────────────────
function makeIdLink(row) {
  if (!row.id) return '—';
  if (db_dim === '3D') {
    // Materials Project
    return `<a href="https://next-gen.materialsproject.org/materials/${row.id}" target="_blank" rel="noopener">${row.id}</a>`;
  }
  // 2D sources
  if (row.src === '2DMatPedia') {
    return `<a href="http://www.2dmatpedia.org/twodmaterials/doc/${row.id}" target="_blank" rel="noopener">${row.id}</a>`;
  }
  if (row.src === 'C2DB') {
    return `<a href="https://c2db.fysik.dtu.dk/material/${row.id}" target="_blank" rel="noopener">${row.id}</a>`;
  }
  // Alexandria — no direct deep link available, just show the ID
  return row.id;
}

// ── Source badge ──────────────────────────────────────────────
const SRC_STYLES = {
  '2DMatPedia': 'db-src-2dmp',
  'C2DB':       'db-src-c2db',
  'Alexandria': 'db-src-alex',
};
function makeSrcBadge(src) {
  const cls = SRC_STYLES[src] || 'db-src-other';
  return `<span class="db-src-badge ${cls}">${src}</span>`;
}
  ...
  for (let r = 1; r <= 10; r++) {
    for (let c = 1; c <= 18; c++) {
      ...
      if (r === 8 || !el) {
        ...
        // keep selected state across rebuilds
        if (db_selected.has(el.sym)) div.classList.add('selected');
        div.dataset.sym = el.sym;
        div.innerHTML = `<span class="pt-sym">${el.sym}</span><span class="pt-num">${el.num}</span>`;
        div.title = el.sym;
        if (isAvail) div.addEventListener('click', () => toggleElement(el.sym));
      }
...
function toggleElement(sym) {
  if (db_selected.has(sym)) { db_selected.delete(sym); } else { db_selected.add(sym); }
  document.querySelectorAll('.pt-cell[data-sym]').forEach(cell => {
    cell.classList.toggle('selected', db_selected.has(cell.dataset.sym));
  });
  updateHint();
  applyFilters();
}
...
function applyFilters() {
  ...
  db_filtered = DB_DATA.filter(row => {
    if (selArr.length > 0) {
      if (!selArr.every(e => row.el.includes(e))) return false;
...
function formatT2Raw(v) {
  // 3D data is stored in seconds; 2D data is stored in ms.
  // Normalise everything to seconds for a unified display.
  const vSec = db_dim === '3D' ? v : v / 1000;
  if (vSec >= 1)     return vSec.toPrecision(4) + ' s';
  if (vSec >= 1e-3)  return (vSec * 1e3).toPrecision(4) + ' ms';
  if (vSec >= 1e-6)  return (vSec * 1e6).toPrecision(4) + ' μs';
  if (vSec >= 1e-9)  return (vSec * 1e9).toPrecision(4) + ' ns';
  return vSec.toExponential(3) + ' s';
}
...
function renderTable() {
  ...
  // Show/hide the Source column based on dim
  const srcCol = document.getElementById('dbColSrc');
  if (srcCol) srcCol.style.display = db_dim === '2D' ? '' : 'none';
  const colSpan = db_dim === '2D' ? 5 : 4;

  const start = (db_page - 1) * DB_PER_PAGE;
  const slice = db_filtered.slice(start, start + DB_PER_PAGE);

  if (slice.length === 0) {
    tbody.innerHTML = `<tr><td colspan="${colSpan}" class="db-td" style="text-align:center;color:var(--text-3);padding:2rem;">No materials match your selection.</td></tr>`;
    return;
  }

  tbody.innerHTML = slice.map((row, i) => {
    const rank = start + i + 1;
    const idCell = `<td class="db-td db-td-id">${makeIdLink(row)}</td>`;
    const srcCell = db_dim === '2D'
      ? `<td class="db-td">${makeSrcBadge(row.src)}</td>`
      : '';
    return `<tr>
...
function renderPagination() {
  ...
  if (totalPages <= 7) {
    ...
    for (let p = Math.max(2, db_page - 1); p <= Math.min(totalPages - 1, db_page + 1); p++) addBtn(p, p, p === db_page);
...
function switchDim(dim) {
  db_dim = dim;
  DB_DATA = dim === '3D' ? DB_DATA_3D : DB_DATA_2D;

  // Update toggle buttons
  document.querySelectorAll('.db-dim-btn').forEach(btn => {
    btn.classList.toggle('selected', btn.dataset.dim === dim);
  });

  // Update table ID column header label
  const idCol = document.getElementById('dbColId');
  if (idCol) idCol.textContent = dim === '3D' ? 'MP ID' : 'ID';

  // Reset element selection and filters (elements present differ per dataset)
  db_selected.clear();
  db_search = '';
...
// ── Init: load both datasets, wire events ─────────────────────
(async function initDatabase() {
  if (!document.getElementById('ptable')) return; // not on database page

  // Load both JSON files in parallel
  try {
    const [res3d, res2d] = await Promise.all([
      fetch('db.json'),
      fetch('db2d.json'),
    ]);
    DB_DATA_3D = await res3d.json();
    DB_DATA_2D = await res2d.json();
  } catch (e) {
    const meta = document.getElementById('dbMeta');
  ...
  if (count3D) count3D.textContent = DB_DATA_3D.length.toLocaleString();
  if (count2D) count2D.textContent = DB_DATA_2D.length.toLocaleString();
  ...
  // Dim toggle buttons
  document.querySelectorAll('.db-dim-btn').forEach(btn => {
    btn.addEventListener('click', () => switchDim(btn.dataset.dim));
  });
